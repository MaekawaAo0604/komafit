-- ============================================================================
-- Fix: get_monthly_calendar_with_patterns - 統合修正
-- ============================================================================
-- 問題1: ts.label → ts.id への修正（20260214184200 で修正済み）が
--         20260216000001 で上書きされ ts.label に戻ってしまっている
-- 問題2: PARTITION BY に position が必要（20260216000001 で修正済み）だが
--         20260214184200 では position が含まれていない
--
-- 本ファイルで両方の修正を統合:
-- - ts.id を使用（ts.label は time_slots に存在しないカラムのため）
-- - PARTITION BY date, time_slot_id, teacher_id, position（複数生徒対応）
-- ============================================================================
...
BEGIN
  ...
  pattern_data AS (
    SELECT
      ...
      ra.time_slot_id,
      ts.id AS time_slot_label,  -- 修正: ts.label → ts.id
  ...
  -- 優先順位に基づいて重複排除
  -- position を PARTITION BY に含めることで、同じスロットの複数生徒を全員表示する
  ranked_data AS (
    SELECT
      combined_data.date,
      combined_data.teacher_id,
      combined_data.teacher_name,
      combined_data.day_of_week,
      combined_data.time_slot_id,
      combined_data.time_slot_label,
      combined_data.is_available,
      combined_data.student_id,
      combined_data.student_name,
      combined_data.student_grade,
      combined_data.subject,
      combined_data.position,
      combined_data.pattern_id,
      combined_data.data_source,
      combined_data.exception_type,
      ROW_NUMBER() OVER (
        PARTITION BY combined_data.date, combined_data.time_slot_id, combined_data.teacher_id, combined_data.position
        ORDER BY combined_data.priority ASC  -- 優先度が低い値（1=最高）が先
      ) AS row_num
    FROM combined_data
  )
  ...
  FROM ranked_data
  WHERE ranked_data.row_num = 1  -- 各日付・コマ・講師・positionの組み合わせで優先度が最も高いレコードのみ
  ORDER BY
    ranked_data.date ASC,
    ranked_data.time_slot_id ASC,
    ranked_data.teacher_name ASC,
    ranked_data.position ASC;

END;
$$;

GRANT EXECUTE ON FUNCTION get_monthly_calendar_with_patterns(INTEGER, INTEGER, UUID) TO authenticated;

-- 関数のコメント
COMMENT ON FUNCTION get_monthly_calendar_with_patterns IS '定期授業パターンを月内の日付に展開し、個別アサインと例外処理を統合する。優先順位: 例外処理 > 個別アサイン > パターン。講師は自分のデータのみ、管理者・viewerは全データを取得可能。同じスロットに複数の生徒がいる場合は全員表示する。[統合修正: ts.id使用 + position partition]';
