-- ============================================================================
-- Fix: get_monthly_calendar_with_patterns - Show multiple students per slot
-- ============================================================================
-- 問題: PARTITION BY に position が含まれていないため、同じスロットに複数の生徒を
--       アサインしても最初の1人しか表示されない
-- 解決: PARTITION BY を修正して、各 (date, time_slot_id, teacher_id, position) の
--       組み合わせで重複排除を行う
-- ============================================================================
...
BEGIN
  ...
  -- 優先順位に基づいて重複排除
  -- 修正: position を PARTITION BY に追加して、複数の生徒を表示できるようにする
  ranked_data AS (
    SELECT
      combined_data.date,
...
-- 関数のコメント
COMMENT ON FUNCTION get_monthly_calendar_with_patterns IS '定期授業パターンを月内の日付に展開し、個別アサインと例外処理を統合する。優先順位: 例外処理 > 個別アサイン > パターン。講師は自分のデータのみ、管理者・viewerは全データを取得可能。同じスロットに複数の生徒がいる場合は全員表示する。';
